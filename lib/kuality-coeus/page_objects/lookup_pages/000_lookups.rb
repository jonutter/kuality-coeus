class Lookups < BasePage
  ...
  def self.url_info(title, class_name)
    page_url "#{$base_url}portal.do?channelTitle=#{title}&channelUrl=#{$base_url[/.+(com|org)/]}:/kc-dev/kr/lookup.do?methodToCall=start&businessObjectClassName=org.kuali.#{class_name}&docFormKey=88888888&includeCustomActionUrls=true&returnLocation=#{$base_url[/.+(com|org)/]}:/kc-dev/portal.do&hideReturnLink=true"
  end
