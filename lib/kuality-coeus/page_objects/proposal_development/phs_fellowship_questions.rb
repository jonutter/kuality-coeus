class PHSFellowshipQuestions < ProposalDevelopmentDocument
  ...
  PHS_SELECT_LIST_NAMES = [:field_of_training, :type_of_degree, :support_level, :support_recipient,
                           :prior_support_award_level, :prior_support_award_recipient]
  [26,30,38,39,
   47,48
  ].each_with_index do |num, index|
    element(PHS_SELECT_LIST_NAMES[index]) { |b| b.frm.select(name: "s2sQuestionnaireHelper.answerHeaders[0].answers[#{num}].answer") }
  end

  PHS_TEXT_FIELD_NAMES = [:expected_degree_completion_date, :support_start_date, :support_end_date, :support_grant_number,
  ...
  [29,41,43,45,
   ...
   81,82,83
    ...
    element(PHS_TEXT_FIELD_NAMES[index]) { |b| b.frm.text_field(id: "s2sQuestionnaireHelper.answerHeaders[0].answers[#{num}].answer") }
  end
