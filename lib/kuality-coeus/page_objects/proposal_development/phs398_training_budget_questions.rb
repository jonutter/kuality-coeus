class PHS398TrainingBudgetQuestions < ProposalDevelopmentDocument
  ...
  0.upto(4) do |x|
    ...
    0.upto(7) do |n|
      element("bp#{x+1}_full_term_nondegree_postdocs_#{x}".to_sym) { |b| b.frm.text_field(id: "s2sQuestionnaireHelper.answerHeaders[0].answers[#{(15+n)+(x*55)}].answer") }
      element("bp#{x+1}_short_term_nondegree_postdocs_#{x}".to_sym) { |b| b.frm.text_field(id: "s2sQuestionnaireHelper.answerHeaders[0].answers[#{(24+n)+(x*55)}].answer") }
